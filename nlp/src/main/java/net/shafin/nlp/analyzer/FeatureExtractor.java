...
public class FeatureExtractor {
    ...
    public static double getNounFrequency(String phrase, IndexService indexService) {
        BanglaWordAnalyzer wordAnalyzer = new BanglaWordAnalyzer(new StringReader(phrase));
        List<String> wordTokens = wordAnalyzer.getTokenList();
        wordAnalyzer.close();

        double numTokens = wordTokens.size();
        double numNoun = 0;
        for (String token : wordTokens) {
            if (indexService.isExistNoun(token)) {
                numNoun++;
            }
        }
        return numNoun / numTokens;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getNounFrequency(phrase, new IndexService()));
    }
}
